...
#
# num_tensors is kept >= 1: an empty batch (nested_tensor([])) is a degenerate
# torch-side layout and is not part of this operator's contract.
#
...
def _make_component(dtype, shape, value_range):
    """Build one nested-tensor component via the shared value-range helper.

    Unsigned integer dtypes cannot represent the negative bound of ranges such
    as ``["-1", "0"]`` (``make_input`` raises RuntimeError on the underlying
    generator), so negative bounds are clamped to 0 for them. The operator
    never reads the stored values, only the layout/dtype have to stay valid.
    """
    if (
        dtype == torch.bool
    ...
    low = max(tu.resolve_bound(value_range[0], dtype), 0.0)
    high = max(tu.resolve_bound(value_range[1], dtype), 0.0)
    if low == high:
        return torch.full(shape, int(low), dtype=dtype, device=flag_gems.device)
    return torch.testing.make_tensor(
        shape, dtype=dtype, device=flag_gems.device, low=int(low), high=int(high)
    )
...
def _nested_dtype_supported(dtype):
    ...
    try:
        low = _make_component(dtype, (2, 2), ["0", "1"])
        high = _make_component(dtype, (3, 2), ["0", "1"])
...
def _make_input(num_tensors, num_dims, dtype, seed=0, value_range=_DEFAULT_VALUE_RANGE):
    ...
    components = [
        _make_component(dtype, (length,) + (4,) * (num_dims - 1), value_range)
...
def _make_strided_view_input(dtype, device=None):
    ...
    # _make_component builds on the shared test device; the contiguous buffer
    # can always be moved to ``device``. Its values are never read.
    buf = _make_component(dtype, (64,), _DEFAULT_VALUE_RANGE).to(device)
...
def test__nested_tensor_storage_offsets_uniform(dtype):
    ...
    num_tensors = 6
    components = [
        _make_component(dtype, (4, 4, 4), _DEFAULT_VALUE_RANGE)
...
def test__nested_tensor_storage_offsets_with_empty_components(dtype):
    ...
    for _ in range(num_tensors):
        ...
        components.append(_make_component(dtype, (length, 4), _DEFAULT_VALUE_RANGE))
    inp = torch.nested.nested_tensor(components, device=flag_gems.device)
    ref_inp = utils.to_reference(inp)
...
def test__nested_tensor_storage_offsets_nan_inf(dtype):
    ...
    components = []
    for length in lengths:
        comp = _make_component(dtype, (length, 4), _DEFAULT_VALUE_RANGE)
        flat = comp.reshape(-1)
        flat[0] = float("nan")
        if flat.numel() > 1:
            flat[1] = float("inf")
...
def test__nested_tensor_storage_offsets_non_nested_raises(dtype):
    ...
    inp = _make_component(dtype, (4, 4), _DEFAULT_VALUE_RANGE)
    ref_inp = utils.to_reference(inp)
