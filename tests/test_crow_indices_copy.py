...
# batch_dims + (n_compressed + 1,) int64 compressed-row index array of a sparse
# row-compressed tensor (CSR or BSR) as a fresh, contiguous, independent copy.
# It is the view_copy counterpart of aten::crow_indices, whose native body is
# ``crow_indices(self).clone(contiguous)``: unlike aten::crow_indices the result
# does NOT alias the input's internal crow storage.
#
# The operator is a sparse-metadata accessor, so the regular-operator dimensions
# are adapted as follows:
#   * shapes (tu.selected_shapes): every required rank is mapped onto a
#     row-compressed layout the operator accepts -- rank 0/1 -> 2-D, rank >= 2 ->
#     batched CSR with the leading dims as batch dims -- plus dedicated
#     CSR/BSR layout cases (single row/column, square, full, empty nnz == 0,
#     ranks 2-5 and block shapes that do not divide the matrix).
#   * value ranges (tu.selected_ranges): the returned crow copy never depends on
...
#     a fresh int64 metadata tensor, so there is nothing to broadcast against or
#     to differentiate.
#
# Copy semantics are checked on every case: the result must equal the raw crow
# array, must be a fresh contiguous int64 tensor that does NOT alias the input's
# internal crow storage, and the input must not be mutated.

# (layout, size, nnz, blocks) core cases: 2-D CSR (incl. single-row and
# single-column), batched CSR, 2-D BSR with varied block shapes, batched BSR,
# and empty (nnz == 0) CSR/BSR.
_CROW_CORE = [
    ("csr", (5, 4), 6, None),
    ("csr", (4, 1), 3, None),
    ("csr", (1, 5), 2, None),
...
def _crow_cases():
    ...
    return _CROW_CORE + _CROW_ALL


def _crow_value_range_cases():
...
def _shape_to_crow_case(shape):
    ...
    stays cheap even for logically large shapes: the returned crow array has one
    entry per compressed row, so its size is driven by the row extent, not nnz.
    """
    if len(shape) == 0:
        size = (1, 1)
    elif len(shape) == 1:
...
def _spec_shape_cases():
    ...
    return [_shape_to_crow_case(shape) for shape in tu.selected_shapes()]
...
def _random_crow(n_compressed, nnz, gen):
    ...
    # degenerate [0, nnz]. Repeated split points leave empty rows, which is valid
...
def _make_values(dtype, values_shape, value_range, gen):
    ...
    # which already creates the tensor on flag_gems.device; the returned crow
    # copy never depends on them, so every per-dtype range can be exercised
    # through this one constructor. bool ignores the range (deterministic random
    # 0/1 keeps construction reproducible).
    if dtype == torch.bool:
        return torch.randint(0, 2, values_shape, dtype=dtype, generator=gen).to(
            flag_gems.device
        )
    # Values are irrelevant to the returned metadata, so a range that clamps to a
    # single representable value for this dtype (e.g. uint8 with [-1, 0]) is
...
def _expected_crow_shape(case):
    # crow_indices_copy returns the compressed-row pointer array: one entry per
    # compressed row plus the sentinel, i.e. n_compressed + 1. For CSR that is
    # nrows + 1; for BSR it is the number of row blocks + 1. Batched layouts
    # preserve the leading batch dims.
...
def _reference_crow_indices_copy(inp):
    ...
    # for every storage dtype used here, so no composed simulation is used.
    return torch.ops.aten.crow_indices_copy(inp)


def _reference_crow_indices_copy_out(inp, out):
    # The .out contract materializes the int64 crow array into out and returns
    # out itself; ATen enforces the int64 out dtype, so the reference calls the
...
def _resolve_gems_op():
    ...
    # process-local override injected by KernelGen for this run wins. Resolution
    # order: (1) override, (2) the direct flag_gems.crow_indices_copy callable,
    # (3) LookupError.
...
def test_crow_indices_copy_spec_shapes(case, dtype):
    ...
    # onto a CSR workload the operator accepts. The logically large shapes keep a
...
@pytest.mark.crow_indices_copy
@pytest.mark.parametrize("case", _crow_value_range_cases())
...
def test_crow_indices_copy_value_ranges(case, value_range, dtype):
    # Value-range coverage: the metadata output (the fresh crow int64 copy) is
    # independent of the stored values, so every per-dtype range -- including the
    # extreme [min, 0] and [0, max] magnitudes -- must be accepted and must not
...
def test_crow_indices_copy_out_value_ranges(case, value_range, dtype):
    # Same sweep through the .out overload: the int64 crow copy written into out
...
def test_crow_indices_copy_empty_bsr(dtype):
    # nnz == 0 for BSR: col and values are empty, but crow_indices_copy must
...
def _uncoalesced_csr(dtype):
    # The (0, 0) entry is duplicated (cols[0] == cols[1] in row 0), which leaves
    # the CSR tensor with repeated entries; crow_indices_copy must still return
    # exactly the stored crow array, unchanged, as an independent copy (never a
    # coalesced/sorted structure and never an alias). Row 0 holds 3 entries for
    # columns [0, 0, 2], so a coalescing implementation would visibly change the
    # stored structure.
    shape = (4, 3)
...
def test_crow_indices_copy_negative_dense():
    # crow_indices_copy is a row-compressed-sparse-only metadata accessor: a
    # dense tensor has no compressed-row storage, so both the reference and the
    # candidate must reject it.
    inp = tu.make_input(torch.float32, (3, 4), ["-1", "1"])
    with pytest.raises((RuntimeError, TypeError)):
        _reference_crow_indices_copy(utils.to_reference(inp.clone()))
    with pytest.raises((RuntimeError, TypeError)):
...
def test_crow_indices_copy_negative_coo():
    # Sparse (COO) is a distinct backend key from SparseCsr; crow_indices_copy
...
def test_crow_indices_copy_negative_non_tensor():
    ...
    with pytest.raises((RuntimeError, TypeError)):
        _reference_crow_indices_copy(3.14)
...
def test_crow_indices_copy_out_negative_dense():
    ...
    with pytest.raises((RuntimeError, TypeError)):
        _reference_crow_indices_copy_out(utils.to_reference(inp.clone()), out)
    with pytest.raises((RuntimeError, TypeError)):
...
def test_crow_indices_copy_out_negative_wrong_dtype():
    ...
    with pytest.raises(RuntimeError):
        _reference_crow_indices_copy_out(ref_inp, ref_out)
    with pytest.raises((RuntimeError, TypeError)):
        _resolve_gems_op_out()(inp, out=out)
