...
#     the returned array.
#   * dtypes: the declared sparse CSC/BSC storage dtypes, including int8,
...
def test_ccol_indices_copy_negative_dense():
    ...
    with pytest.raises((RuntimeError, TypeError)):
        torch.ops.aten.ccol_indices_copy(tu.to_reference(inp.clone()))
    with pytest.raises((RuntimeError, TypeError)):
...
def test_ccol_indices_copy_out_negative_dense():
    ...
    with pytest.raises((RuntimeError, TypeError)):
        torch.ops.aten.ccol_indices_copy.out(tu.to_reference(inp.clone()), out=out)
    with pytest.raises((RuntimeError, TypeError)):
