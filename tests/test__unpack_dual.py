...
# created at an active forward-mode AD ``level`` (by ``aten._make_dual``), it
# returns the primal as an aliasing view (Tensor(a): same shape, strides,
# storage offset, dtype and data_ptr) and the tangent registered at that level.
# The level must already be active, so the native implementation is only
# callable inside ``torch.autograd.forward_ad.dual_level()``; the caller always
# takes the level from the context because its exact value varies across torch
# versions. On a plain tensor with no forward tangent at ``level`` the tangent
# component is None while the primal view is still returned. No arithmetic
# happens: the op is pure metadata/view manipulation, so primal values, tangent
# values and the alias contract are all asserted below.
#
# Coverage follows the regular-operator spec adapted to a metadata/view op:
#   * shape levels: tu.selected_shapes() (0~5 dims, selected by --quick);
...
#     are covered through the plain tensor (tangent-None) path;
...
# performs no arithmetic, its primal output is an aliasing view of the input
...
# Dual tensors can only be constructed over floating-point/complex primals
# (aten raises on int/bool). complex32 is excluded because the comparison
# helpers cannot materialize ComplexHalf; complex64 covers the complex branch.
DUAL_DTYPES = [
    d
...
# Representative ranks for the full value-range sweep (0-dim, 1-dim, 3-dim); the
# shape-level sweep already covers every rank in the active level.
...
def _resolve_gems_op():
    return tu.resolve_gems_op("_unpack_dual", getattr(flag_gems, "_unpack_dual", None))
...
def test__unpack_dual_dual_tensor(shape, dtype):
    ...
    ref_primal = utils.to_reference(primal, independent=True)
    ref_tangent = utils.to_reference(tangent, independent=True)
...
def test__unpack_dual_mutation(shape, dtype):
    ...
    # mutates anything.
    primal = _make_input(dtype, shape, ["-1", "1"])
    ref_primal = utils.to_reference(primal.clone(), independent=True)
    tangent = _make_input(dtype, shape, ["-1", "1"])
    ref_tangent = utils.to_reference(tangent, independent=True)
...
if tu.LEVEL == "all":
    ...
    def test__unpack_dual_special_values(dtype):
        ...
        ref_primal = utils.to_reference(values, independent=True)
