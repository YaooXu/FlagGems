...
# (N, C_in, D, H, W), ``weight`` is (C_in, C_out, kD, kH, kW) and ``kernel_size``
# must match the weight spatial dims. The output is (N, C_out, D_out, H_out,
# W_out) with
#   D_out = (D - 1)*sD - 2*pD + dil_d*(kD - 1) + out_pad_d + 1
# and likewise for H and W (output_padding must be < stride per dim). Each
# (input, weight, kernel_size, stride, padding, output_padding, dilation) tuple
# below is one distinct parametrized workload: they cover 1x1x1/2x2x2/3x3x3
# kernels, stride 1/2, padding 0/1/2, output_padding 0/1, dilation 1/2, and
...
def _make_conv_inputs(inp_shape, weight_shape, with_bias, dtype):
    ...
    # For transposed conv3d the weight is (C_in, C_out, kD, kH, kW), so the
    # bias length is the second (output-channel) dim.
...
def _assert_close(res_out, ref_out, dtype):
    ...
    # the transposed conv in the input dtype: fp16/bf16 keep at most fp16/bf16
    # precision per add, so the native op itself deviates from the fp64
    # reference by up to ~3.4e-2 (fp16) and ~1.7e-1 (bf16) on the larger 3D
    # reductions (up to C_in*kD*kH*kW = 108 terms). Measure the deviation over
    # multiple seeds and shapes: fp16 -> 6e-2 and bf16 -> 3.5e-1 give a ~2x
    # margin; fp32 with TF32 disabled (set at the top of each test) stays at
    # ~2e-6, comfortably inside the default 1e-4.
