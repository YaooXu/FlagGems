...
# aten::atleast_2d is a pure view/identity op: 0-dim tensors are reshaped to
# (1, 1), 1-dim tensors to (1, N) (both views), and tensors with two or more
# dimensions are returned as-is. No arithmetic is performed, so the result must
# match bit-for-bit, alias the input, and every dtype the op supports is
...
# boundary magnitudes per dtype (int/bool are exact; float uses equal_nan).
ATLEAST_2D_DTYPES = utils.ALL_FLOAT_DTYPES + utils.ALL_INT_DTYPES + utils.BOOL_TYPES

# The shared shape levels cover the dim boundary that drives the op: 0-dim ->
# (1, 1), 1-dim -> (1, N), 2-dim identity and higher-dim identity.
_ATLEAST_2D_SHAPES = tu.selected_shapes()

# Backward shapes stay small (the autograd graph is built on the reference and
# the comparison is elementwise); 0-dim/1-dim exercise the shape-changing views.
_ATLEAST_2D_BACKWARD_SHAPES = [(), (3,), (16, 64), (7, 13, 29)]


def _resolve_gems_op():
    # Resolution order: (1) the process-local override injected by KernelGen,
    # (2) the direct flag_gems.atleast_2d callable, (3) None -> the test falls
    ...
    # are resolved through the shared public operator name "atleast_2d".
    try:
...
@pytest.mark.parametrize("shape", _ATLEAST_2D_SHAPES)
@pytest.mark.parametrize("value_range", tu.selected_ranges())
@pytest.mark.parametrize("dtype", ATLEAST_2D_DTYPES)
def test_atleast_2d_value_ranges(shape, value_range, dtype):
    inp = tu.make_input(dtype, shape, value_range)
    ref_inp = utils.to_reference(inp)

    ref_out = torch.ops.aten.atleast_2d(ref_inp)
    res_out = _apply_atleast_2d(inp)

    assert res_out.shape == ref_out.shape
    assert res_out.dtype == ref_out.dtype
    # atleast_2d is a view op: the result must alias the input.
...
def test_atleast_2d_nan_inf(dtype):
...
def test_atleast_2d_complex(dtype):
    # atleast_2d also supports complex tensors (a pure view: real and imaginary
...
@pytest.mark.parametrize("value_range", tu.selected_ranges())
@pytest.mark.parametrize("dtype", ATLEAST_2D_DTYPES)
def test_atleast_2d_sequence(shape, value_range, dtype):
    # Mix a 0-dim scalar, a 1-dim tensor and the current shape so the sequence
    # overload exercises all three paths: scalar -> (1, 1), 1-dim -> (1, N)
    # and the >= 2-dim identity.
    inp = [
    ...
    for res, ref, src in zip(res_out, ref_out, inp):
        ...
        # atleast_2d is a view op: each result must alias its input.
...
def test_atleast_2d_sequence_empty():
    ...
    res_out = _apply_atleast_2d([])
    assert len(ref_out) == 0
    assert len(res_out) == 0


@pytest.mark.atleast_2d_backward
@pytest.mark.parametrize("shape", _ATLEAST_2D_BACKWARD_SHAPES)
@pytest.mark.parametrize("dtype", utils.FLOAT_DTYPES)
def test_atleast_2d_backward(shape, dtype):
    inp = tu.make_input(dtype, shape, ["-1", "1"]).requires_grad_()
    ref_inp = utils.to_reference(inp)

    # atleast_2d is a view: the gradient of sum(atleast_2d(x)) is all-ones in
    # x's shape on both the shape-changing (0-dim/1-dim) and identity paths.
    ref_out = torch.ops.aten.atleast_2d(ref_inp)
