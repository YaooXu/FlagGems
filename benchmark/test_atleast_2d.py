...
def _sequence_case_fn(shape, dtype):
    del dtype
    # Mix a 0-dim scalar, a 1-dim tensor and the current benchmark shape so the
    # sequence overload exercises the scalar -> (1, 1), 1-dim -> (1, N) and
    # >= 2-dim identity paths.
    seq_shapes = [(), (3,), shape]
    yield base.BenchmarkCasePlan(
...
class Atleast2DBenchmark(base.GenericBenchmark):
    def set_shapes(self, shape_file_path=None):
        super().set_shapes(shape_file_path)
        # atleast_2d's defining cases are the 0-dim scalar -> (1, 1) and
        # 1-dim -> (1, N) views.
